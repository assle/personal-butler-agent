...
**Goal:** Integrate WeChat Work self-built app callback and group bot webhook push into the Personal Butler Agent.

**Architecture:** New `src/wechat/` module with crypto, message parsing, webhook client, and FastAPI router. Each layer is testable independently before wiring into `main.py`. Conditional registration ensures no breakage when WeChat env vars are absent.

**Tech Stack:** Python 3.13+, cryptography (AES-256-CBC), httpx, FastAPI
...
Run: `uv sync`
Expected: cryptography installed without errors
...
### Task 2: Add WeChat configuration to Settings

**Files:**
- Modify: `src/config.py`

- [ ] **Step 1: Add WeChat settings**
...
### Task 3: Implement crypto.py (TDD)

**Files:**
- Create: `src/wechat/crypto.py`
- Create: `tests/test_wechat_crypto.py`
- Create: `src/wechat/__init__.py` (minimal)
...
Run: `DEEPSEEK_API_KEY=test uv run pytest tests/test_wechat_crypto.py -v`
Expected: FAIL with "ModuleNotFoundError: No module named 'src.wechat.crypto'"
...
### Task 4: Implement messages.py (TDD)

**Files:**
- Create: `src/wechat/messages.py`
- Create: `tests/test_wechat_messages.py`
...
Run: `DEEPSEEK_API_KEY=test uv run pytest tests/test_wechat_messages.py -v`
Expected: FAIL with "ModuleNotFoundError" for messages
...
### Task 5: Implement webhook.py (TDD)

**Files:**
- Create: `src/wechat/webhook.py`
...
Run: `DEEPSEEK_API_KEY=test uv run pytest tests/test_wechat_webhook.py -v`
Expected: FAIL with "ModuleNotFoundError: No module named 'src.wechat.webhook'"
...
### Task 6: Implement router.py (TDD)

**Files:**
- Create: `src/wechat/router.py`
- Create: `tests/test_wechat_router.py`
...
Run: `DEEPSEEK_API_KEY=test uv run pytest tests/test_wechat_router.py -v`
Expected: FAIL with "ModuleNotFoundError: No module named 'src.wechat.router'"
...
### Task 7: Add XML body support to router

**Files:**
- Modify: `src/wechat/router.py`
- Modify: `tests/test_wechat_router.py`

- [ ] **Step 1: Update router to also accept XML body and use messages.py for parsing**

Edit `src/wechat/router.py` — replace the `receive_message` function to accept both JSON and XML bodies:
...
- [ ] **Step 2: Run all wechat tests to verify**

Run: `DEEPSEEK_API_KEY=test uv run pytest tests/test_wechat_router.py tests/test_wechat_messages.py -v`
Expected: 9 passed (4 router + 5 messages rounded up)
...
### Task 8: Update wechat/__init__.py with public exports
...
Run: `uv run python -c "from src.wechat import WechatWebhookClient, create_wechat_router; print('imports ok')"`
Expected: "imports ok"
...
- [ ] **Step 1: Update main.py to conditionally register wechat router and webhook client**
...
- [ ] **Step 2: Verify the app starts without WeChat config**

Run: `DEEPSEEK_API_KEY=test uv run uvicorn src.main:app --host 0.0.0.0 --port 8000 & sleep 3 && curl -s http://localhost:8000/docs | head -5 && kill %1`
Expected: should see OpenAPI docs, no wechat routes listed
...
Run: `DEEPSEEK_API_KEY=test uv run pytest -q -v`
Expected: all tests pass, no regressions

- [ ] **Step 2: Verify debug endpoint still works**
...
```
Expected: JSON response with intent/confidence/response fields
